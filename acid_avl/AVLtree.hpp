...
namespace AVLtree {
    ...
    public:
        ...
        AVLiterator(const pointer &smart_ptr, const pointer &end) noexcept : ptr(smart_ptr), end_(end),
            state(states::VALID) {}
    ...
    private:
        ...
        void init_tree(smart_ptr &node, const value_type &value) {
            ...
            iterator tmp_1(node->left, this->sentinel);
            iterator tmp_2(this->sentinel, this->sentinel);
            ...
            if (node->data.first > key) {
                ...
                if ((!(node->left)) || (!(node->right))) {
                    ...
                    if (!(tmp)) {
                        ...
                        if (this->sentinel->parent == node) {
                            this->sentinel->parent = tmp;
                        }

                        tmp->parent = node->parent;
                        if (node.get_core()->ref_count > 1) node->state = states::REMOVED;
                        //smart_ptr node_ = node;
